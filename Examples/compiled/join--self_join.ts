...
const admins_0 = new NamedSource(new SelectQuerySource(new JoinSource(users_0, new NamedSource(users_0, "admins"), condition_0, JoinType.Inner), {
    fieldFilter: (field: QualifiedName) => ["users.id", "users.firstName", "users.adminId", "admins.id", "admins.firstName"]
        // TODO: Fix for namespace 
        .includes(field.name) || ["users.id", "users.firstName", "users.adminId", "admins.id", "admins.firstName"].includes(`${field.namespace}.${field.name}`),
    missingFields: (fields: QualifiedName[]) => {
        const fieldNames = fields.map((field) => field.name);
        const qualifiedNames = fields.map((field) => field.namespace + "." + field.name);
        // TODO: Fix for namespace 
        const result = ["users.id", "users.firstName", "users.adminId", "admins.id", "admins.firstName"].filter((field) => !(qualifiedNames.includes(field) || fieldNames.includes(field)));
        return { result, position: "Identifier \"users\" (line 2, column 17)" };
    }
}, undefined), "admins");
const output_0 = new FileOutput("outputs/join--self_join.csv");
admins_0.registerConsumer(output_0);
