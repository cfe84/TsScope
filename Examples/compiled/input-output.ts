...
function createStream() {
  ...
  const OUTPUT_FILE__0 = FileOutputFactory.create("outputs/input-output--load_and_output.csv");
  SOURCE__input_0.registerConsumer(OUTPUT_FILE__0);
  
  const OUTPUT_FILE__1 = FileOutputFactory.create("outputs/input-output--direct_copy.csv");
  FileSourceFactory.create("inputs/users.csv", new RecordMapper_1()).registerConsumer(OUTPUT_FILE__1);
