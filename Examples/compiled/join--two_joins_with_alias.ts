...
const withCountry_0 = new NamedSource(new SelectQuerySource(new JoinSource(new JoinSource(new NamedSource(input_0, "users"), roles_0, condition_1, JoinType.Inner), country_0, condition_0, JoinType.Inner), star, (record: any) => {
    record = recordToObject(record);
    Object.assign(globalThis, record);
    const res = // Condition must be on new line to accomodate for the tsIgnore flag
